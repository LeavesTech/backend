...
type RestOf<T extends IExam | NewExam> = Omit<T, 'startedAt' | 'endAt'> & {
...
export class ExamService {
  ...
  protected convertDateFromClient<T extends IExam | NewExam | PartialUpdateExam>(exam: T): RestOf<T> {
    return {
      ...exam,
      startedAt: exam.startedAt?.toJSON() ?? null,
      endAt: exam.endAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restExam: RestExam): IExam {
    return {
      ...restExam,
      startedAt: restExam.startedAt ? dayjs(restExam.startedAt) : undefined,
      endAt: restExam.endAt ? dayjs(restExam.endAt) : undefined,
    };
  }
