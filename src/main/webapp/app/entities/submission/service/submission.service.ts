...
type RestOf<T extends ISubmission | NewSubmission> = Omit<T, 'submissionDate'> & {
...
export class SubmissionService {
  ...
  protected convertDateFromClient<T extends ISubmission | NewSubmission | PartialUpdateSubmission>(submission: T): RestOf<T> {
    return {
      ...submission,
      submissionDate: submission.submissionDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSubmission: RestSubmission): ISubmission {
    return {
      ...restSubmission,
      submissionDate: restSubmission.submissionDate ? dayjs(restSubmission.submissionDate) : undefined,
    };
  }
