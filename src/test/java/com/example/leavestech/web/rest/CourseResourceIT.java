...
import static com.example.leavestech.domain.CourseAsserts.*;
...
import com.example.leavestech.repository.CourseRepository;
import com.example.leavestech.service.dto.CourseDTO;
import com.example.leavestech.service.mapper.CourseMapper;
...
class CourseResourceIT {
    ...
    public static Course createEntity(EntityManager em) {
        Course course = new Course().code(DEFAULT_CODE).name(DEFAULT_NAME).term(DEFAULT_TERM);
        // Add required entity
        Teacher teacher;
        if (TestUtil.findAll(em, Teacher.class).isEmpty()) {
            teacher = TeacherResourceIT.createEntity();
            em.persist(teacher);
            em.flush();
        } else {
    ...
    public static Course createUpdatedEntity(EntityManager em) {
        Course updatedCourse = new Course().code(UPDATED_CODE).name(UPDATED_NAME).term(UPDATED_TERM);
        // Add required entity
        Teacher teacher;
        if (TestUtil.findAll(em, Teacher.class).isEmpty()) {
            teacher = TeacherResourceIT.createUpdatedEntity();
            em.persist(teacher);
            em.flush();
        } else {
            teacher = TestUtil.findAll(em, Teacher.class).get(0);
        }
        updatedCourse.setOwner(teacher);
    ...
    void putExistingCourse() throws Exception {
        ...
        updatedCourse.code(UPDATED_CODE).name(UPDATED_NAME).term(UPDATED_TERM);
    ...
    void fullUpdateCourseWithPatch() throws Exception {
        ...
        partialUpdatedCourse.code(UPDATED_CODE).name(UPDATED_NAME).term(UPDATED_TERM);
